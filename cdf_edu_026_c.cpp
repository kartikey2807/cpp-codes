...
lld a, b;
bool fits (lld x1, lld y1, lld x2, lld y2)
{
	if ((x2+x1) <= a && max(y2,y1) <= b)
		return true;
	if ((y2+y1) <= b && max(x2,x1) <= a)
		return true;
	return false;
}
int main ()
{
      ...
      for(lld i = 0; i < n; i += 1){
      	for(lld j = 0; j < n; j += 1){
      		if (j == i)
      			continue;
      		if (fits(X[i],Y[i],X[j],Y[j]) || fits(X[i],Y[i],Y[j],X[j]) || fits(Y[i],X[i],X[j],Y[j]) || fits(Y[i],X[i],Y[j],X[j]))
